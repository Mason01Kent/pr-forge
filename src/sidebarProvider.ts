        ...
        webviewView.webview.onDidReceiveMessage((msg: WebviewToExtMsg) => {
            switch (msg.command) {
                ...
                case 'setApiKey':
                    ...
                    break;
                case 'removeApiKey':
                    this._callbacks.onRemoveApiKey();
                    break;
                case 'showTools':
...
<div id="tools-view">
  ...
  <!-- Onboarding / setup card (shown until config + key are sorted) -->
  <div class="onboard-card" id="onboard-card" style="display:none">
    <div class="onboard-title" id="onboard-title">Set up PR Forge</div>
    <div class="onboard-text" id="onboard-text"></div>
    <button class="btn btn-primary" id="btn-onboard-primary"><span id="btn-onboard-primary-label">Set Up</span></button>
    <div class="onboard-note">No API key required — PR Forge generates PR bodies from templates without one. Add a key to unlock AI-written descriptions and code review.</div>
  </div>
...
<div id="preview-view" style="display:none">
  ...
  <div class="preview-actions" id="preview-actions">
    <button class="btn-preview-action" id="btn-preview-copy-title" style="display:none" title="Copy the generated title">${ic.copy}<span>Copy Title</span></button>
    <button class="btn-preview-action" id="btn-preview-copy-body" title="Copy the generated body">${ic.copy}<span>Copy Body</span></button>
    <button class="btn-preview-action btn-preview-draft" id="btn-preview-draft" style="display:none" title="Create or update a draft PR">${ic.draft}<span id="btn-preview-draft-label">Submit Draft</span></button>
    <button class="btn-preview-action btn-preview-submit" id="btn-preview-submit" style="display:none" title="Submit or update the PR">${ic.submit}<span id="btn-preview-submit-label">Submit PR</span></button>
  </div>
  ...
  <div class="regen-bar" id="regen-bar" style="display:none">
    ...
    <div class="regen-input-row">
      ...
      <button class="btn-regen" id="btn-regen" title="Regenerate the draft using your instruction">${ic.sync}<span>Refine</span></button>
    </div>
  </div>
</div>
...
<script nonce="${nonce}">
  ...
  function updateWorkflowHint(state, noAuth, noFeatureBranch, hasSubmittedPr, hasOpenPr) {
    const hint = el('workflow-hint');
    const text = el('workflow-hint-text');
    let message = '';

    if (!state.configExists) {
      message = 'Initialize the project config to tailor generation and submission for this repo.';
    } else if (noFeatureBranch) {
      message = 'Switch to a feature branch first so PR Forge can generate a draft safely.';
    } else if (!noAuth && !state.providerKeySet) {
      message = 'No API key configured. Generate PR Body still works in template mode; Set API Key unlocks AI descriptions and review.';
    } else if (!state.bodyExists) {
      message = 'Generate PR Body first. That unlocks submit, refine, and preview actions.';
    } else if (!hasSubmittedPr && !hasOpenPr) {
      message = 'Draft ready. Submit PR when you want to open or update the remote PR.';
    } else if (hasSubmittedPr) {
      message = state.readinessState === 'ready'
        ? 'PR is submitted and ready. Review threads or open the PR in your browser.'
        : 'PR is submitted. Check readiness, review threads, or post the latest review.';
    }
  ...
  function applyState(state) {
    ...
    // Onboarding card: prominent, actionable setup until config + key are handled.
    // No need to open Setup & Tools for first-run setup.
    const onboard = el('onboard-card');
    const onboardPrimary = el('btn-onboard-primary');
    if (!state.configExists) {
      el('onboard-title').textContent = 'Set up PR Forge for this repo';
    ...
    updateWorkflowHint(state, noAuth, !state.currentBranch || state.currentBranch === 'HEAD' || state.currentBranch === state.baseBranch, hasSubmittedPr, hasOpenPr);
